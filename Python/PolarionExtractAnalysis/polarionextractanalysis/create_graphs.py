        ...
        with logger_config.stopwatch_with_label(f"create_baregraph_work_item_number_by_status_by_type, type {work_item_type.name}"):
            ...
            df = pandas.DataFrame(
                [
                    {
                        "Status": status.name,
                        "Number of work items": len([work_item for work_item in work_items if work_item.attributes.status == status]),
                    }
                    for status in found_status
                ]
            )
            df.plot.bar(use_index=True, x="Status", y="Number of work items", rot=90, color="#9a95d1")
