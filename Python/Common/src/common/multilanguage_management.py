...
class MultilanguageManagement():
    ...
    def __init__(self, translations_json_file_path:str, default_language:str, current_language:str = ""):
        # Langue par défaut (français) pour les traductions manquantes
        self._default_language = default_language
        self._current_language = current_language if current_language != "" else default_language
    ...
    def _load_translations(self, file_path:str)->None:
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            logger_config.print_and_log_info(f"Translation file {file_path} loaded. Number of translations:{len(self._translations)}")
    ...
    def get_translation(self, lang:str, key:str)->str:
        ...
        if translation_key_found is None:
            logger_config.print_and_log_error(f"Could not find translation key {key}. Returns it")
            return key
        ...
        if translation_for_required_language is not None:
            ...
            logger_config.print_and_log_info(f"No translation in language {lang} found for key {key}. Other languages found:{translation_key_found.keys()}")
            translation_for_default_language = translation_key_found.get(self._default_language)
            if translation_for_default_language is not None:
                logger_config.print_and_log_info(f"Return default language {self._default_language} translation for key {key}")
                return translation_for_default_language

            else:
                logger_config.print_and_log_warning(f"Return first found language {self._default_language} translation for key {key}, because neither requested lang {lang} nor {self._default_language} translations were found")
                return key + lang
