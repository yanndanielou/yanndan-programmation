...
class TestMultipleDecodings:

    def test_decode_byte_message(self) -> None:
    ...
    def test_decode_bits_message(self) -> None:
...
class TestDecodeMessageFromBothEnds:
    @pytest.mark.parametrize(
        "hex_string,expecting_last_byte_as_single_int_unsigned",
        [
            ("03h", 3),
            ("01h 04h", 4),
        ],
    )
    def test_decode_end_of_message(self, hex_string: str, expecting_last_byte_as_single_int_unsigned: int) -> None:
        decoded_message = bytes_messages.DecodedBytesMessage.from_hex_string(hex_string)
        assert decoded_message.get_and_remove_last_byte_as_single_int_unsigned() == expecting_last_byte_as_single_int_unsigned

    def test_decoding_left_and_right_extremities_alternatively(self) -> None:
        decoded_message = bytes_messages.DecodedBytesMessage.from_hex_string("03h 01h C1h 6Ah 30h")
        assert decoded_message.get_next_byte_as_single_int_unsigned() == 3
        assert decoded_message.get_and_remove_last_byte_as_single_int_unsigned() == 48
        assert decoded_message.get_next_byte_as_single_int_unsigned() == 1
        assert decoded_message.get_and_remove_last_byte_as_single_int_unsigned() == 106
        assert decoded_message.get_next_byte_as_single_int_unsigned() == 193
