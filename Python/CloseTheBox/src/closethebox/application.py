...
class OneTurn:
    ...
    closed_hatches_during_turns:list[int]
    previously_already_closed_hatches:list[int] = field(default_factory=list)

    def __str__(self) -> str:
        return f"Dices thrown:{self.dices_results}, opened_hatches_before_turn:{self.opened_hatches_before_turn}, closed_hatches_during_turns:{self.closed_hatches_during_turns}"
...
class Application:
    ...
    def compute_dices_combinaisons_with_same_sum(self, two_dices_sum:int, two_dices_results:list[Tuple[int, int]], opened_hatches:list[int], previous_turns:list[OneTurn]|None = None)->list[OneGame]:
        ...
        for dices_results in two_dices_results:
            ...
            for hatches_combinaison in all_hatches_combinaisons:
                ...
                current_turn_with_combinaison = OneTurn(dices_sum=two_dices_sum,
                                                        ...
                                                        closed_hatches_during_turns=hatches_combinaison,
    ...
    def compute_all_possibilities_with_hatches(self, opened_hatches:list[int], previous_turns:list[OneTurn]|None = None)->list[OneGame]:
        ...
        two_dices_all_combinaisons_with_occurences:dict[int,list[Tuple[int, int]]] = Dices.get_two_dices_all_combinaisons_by_sum()

        for two_dices_sum, two_dices_results in two_dices_all_combinaisons_with_occurences.items():
            all_possibilities_with_hatches += self.compute_dices_combinaisons_with_same_sum(two_dices_sum, two_dices_results, opened_hatches, previous_turns)
