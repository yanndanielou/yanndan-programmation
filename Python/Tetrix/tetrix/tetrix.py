...
SCORE_FILE_PATH = os.path.join(os.path.dirname(__file__), ".highscores")
...
class Matris(object):
    def __init__(self):
        ...
        self.highscore = high_scores_utils.load_score(SCORE_FILE_PATH)
    ...
    def update(self, timepassed):
        ...
        self.downwards_speed = constants.BASE_DOWNWARDS_SPEEDS_in_MS ** (1 + self.level / 10.0)

        self.downwards_timer += timepassed
        downwards_speed = self.downwards_speed * 0.10 if any([pygame.key.get_pressed()[pygame.K_DOWN], pygame.key.get_pressed()[pygame.K_s]]) else self.downwards_speed
    ...
    def gameover(self, full_exit=False) -> None:
        ...
        high_scores_utils.write_score(self.score, SCORE_FILE_PATH)
    ...
    def block(self, color, shadow=False):
        ...
        """
        colors = {"blue": (105, 105, 255), "yellow": (225, 242, 41), "pink": (242, 41, 195), "green": (22, 181, 64), "red": (204, 22, 22), "orange": (245, 144, 12), "cyan": (10, 255, 226)}
        ...
        for x in range(posX, posX + len(shape)):
            for y in range(posY, posY + len(shape)):
                if (
                    ...
                    and shape[y - posY][x - posX]  # shape is outside the matrix  # coordinate is occupied by something else which isn't a shadow
                    or copy.get((y, x))
