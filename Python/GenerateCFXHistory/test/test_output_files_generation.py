...
class TestGenerateByProjectInstruction:
    ...
    def test_by_project_and_also_global_all_projects_atspnextlib(self) -> None:
        ...
        assert glob.glob(f"{output_directory_name}/*ATSP*cumulative*.html")
        assert glob.glob(f"{output_directory_name}/*ATSP*cumulative*.png")
        assert glob.glob(f"{output_directory_name}/*ATSP*.json")
