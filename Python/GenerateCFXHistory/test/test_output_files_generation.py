...
class TestGenerateByProjectInstruction:
    ...
    def test_global_all_projects_projects_nextlib(self) -> None:
        ...
        assert not glob.glob(f"{output_directory_name}/*FR_NEXTEO*cumulative*.html")
        assert not glob.glob(f"{output_directory_name}/*FR_NEXTEO*cumulative*.png")
        assert not glob.glob(f"{output_directory_name}/*FR_NEXTEO*.json")

    def test_one_project_only_projects_nextatsplib(self) -> None:
        ...
        ui_and_results_generation.produce_results_and_displays_for_libary(
            ...
            generation_instructions=ui_and_results_generation.GenerationInstructionsForLibary(
                ...
                project_in_case_of_generate_by_project_instruction_one_project="FR_NEXTEO",
                display_output_plots=DISPLAY_OUTPUT,
                dump_all_cfx_ids_in_json=CREATE_JSON_DUMP,
            ),
        )
        ...
        assert not glob.glob(f"{output_directory_name}/*ATSP*cumulative*.html")
        assert not glob.glob(f"{output_directory_name}/*ATSP*cumulative*.png")
        assert not glob.glob(f"{output_directory_name}/*ATSP*.json")
