from stsloganalyzis.archive import decode_action_set_content, decode_message, decode_xml_message
...
class TestRiyl:
    ...
    def test_first_bit(self) -> None:
        ...
        decoded_action_set = decode_action_set_content.ActionSetContentDecoder(csv_file_file_path=csv_file_path).decode(
            decoded_message=decode_message.DecodedMessage(
                message_number=decode_action_set_content.ATS_CC_ACTION_SET_MESSAGE_ID,
                xml_decoded_message=decode_xml_message.DecodedXmlMessage(message_number=decode_action_set_content.ATS_CC_ACTION_SET_MESSAGE_ID, hex_string=first_bit_on_only),
            )
        )
        result = decoded_action_set.action_set_id_with_value_true
        assert len(result) == 1
