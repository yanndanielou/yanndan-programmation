...
class TestDecodeHlfFieldsToDatetime:
    def test_0(self) -> None:
        decoded_datetime = decode_message.XmlMessageDecoder(xml_directory_path="D:/RIYL1/Data/Xml").decode_hlf_fields_to_datetime(
            time_field_value=0, time_offset_value=0, decade_field_value=0, day_on_decade_field_value=0
        )
        assert decode_message.XmlMessageDecoder.decode_hlf_fields_to_datetime(time_field_value=0, time_offset_value=0, decade_field_value=0, day_on_decade_field_value=0) == datetime.datetime(
    ...
    def test_several_data(self, time_field_value: int, time_offset_value: int, decade_field_value: int, day_on_decade_field_value: int, expected_datetime: datetime.datetime) -> None:
        ...
        decoded_hlf = decode_message.XmlMessageDecoder.decode_hlf_fields_to_datetime(
